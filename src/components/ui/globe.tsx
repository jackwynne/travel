		...
		(async () => {
			try {
				if (!ThreeGlobeCtor) {
					...
					ThreeGlobeCtor = (mod as any).default ?? mod;
				}
				if (!didExtendThreeGlobe && ThreeGlobeCtor) {
					extend({ ThreeGlobe: ThreeGlobeCtor });
				...
				if (!globeRef.current && groupRef.current && ThreeGlobeCtor) {
					globeRef.current = new ThreeGlobeCtor();
...
export function WebGLRendererConfig() {
	...
	}, [gl.setPixelRatio, gl.setSize, gl.setClearColor, size.width, size.height]);
